...
from services.kube_client import _load_kube  # 复用你已有的加载逻辑
...
def _parse_mem_to_gb(mem: str) -> float:
    ...
    for u, base in units:
        if s.endswith(u):
            v = float(s[: -len(u)])
            bytes_val = v * base
            return bytes_val / (1000**3)  # GB(10^9) 展示更直观
        ...
        rows.append(
            {
                ...
                # podUsed 做精确统计要按 nodeName 遍历 pods，这里给个可用实现（下面一起算）
                "podUsed": 0,
            }
        )

    # podUsed：统计每个节点当前运行/挂起的 Pod 数量（全命名空间）
    # 这一步会稍慢但能接受（小集群没问题）
    try:
